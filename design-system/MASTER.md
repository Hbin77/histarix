...
| `--globe-outline` | `rgba(27,37,64,0.15)` | 잉크 헤어라인 국경 |
...
- 국가는 **에디토리얼 틴트 채움**(4096×2048 캔버스 텍스처): 기본 상태는 슬롯색을 종이 쪽으로 80% 희석한 속삭임 수준, 호버 52%, **선택 시에만 풀 슬롯색** — 색은 장식이 아니라 의미(포커스)를 나타냄. 국경은 잉크 0.15 헤어라인
- 국가 선택 시 **파라메트릭 3D 모뉴먼트**(높이 0.24)가 실좌표에서 지면 아래로부터 오버슈트 상승(easeOutBack 1.2s)으로 등장
- 모뉴먼트 발밑은 **소프트 컨택트 섀도**(방사형 잉크 그라디언트) — 게임 토큰이 아닌 전시대처럼 접지
