...
| OpenAI GPT API | OpenAI | AI 역사 가이드 챗봇 | 유료 (종량제) |
...
### 4.6 AI 역사 가이드 챗봇 (Histy)

- 오른쪽 하단 플로팅 버튼으로 AI 챗봇 열기/닫기
- GPT API 기반 실시간 스트리밍 응답
- 현재 선택한 국가 컨텍스트를 AI에게 전달하여 맞춤 답변
- 대화 히스토리 유지 (최대 10턴)
- 4개 언어 지원 (한/영/중/일)
- 교육적이고 정확한 역사 정보 제공
...
| POST | `/api/chat` | AI 챗봇 응답 | OpenAI GPT |
| POST | `/api/chat/stream` | AI 챗봇 스트리밍 | OpenAI GPT |
...
- [x] AI 역사 가이드 챗봇 (GPT API - Streaming)
- [ ] 역사 인물 카드
...
| OpenAI GPT API (~100K 토큰/일) | ~₩50,000 |
| Stripe (~100건) | ~₩30,000 |
| **합계** | **~₩180,000/월** |
...
| OpenAI GPT API (~2M 토큰/일) | ~₩1,000,000 |
...
이 프로젝트는 AI와의 적극적 협업을 통해 개발되었습니다.

| AI 활용 영역 | 도구 | 설명 |
|-------------|------|------|
| **기획** | Claude | 프로젝트 구조 설계, 기술 스택 선정, API 아키텍처 |
| **코딩** | Claude Code | 전체 프론트엔드/백엔드 구현, 코드 리뷰 |
| **핵심 기능** | OpenAI GPT API | AI 역사 가이드 챗봇 (실시간 스트리밍) |
| **보안** | Claude Code | SSRF 방지, 입력 검증, CORS 설정 |
| **다국어** | Claude Code | 한/영/중/일 4개 언어 i18n 구현 |
| **문서화** | Claude | README, API 가이드, 기획서 작성 |

자세한 내용은 `docs/AI_협업_가이드.md`를 참조하세요.
